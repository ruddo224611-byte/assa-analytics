...
export async function GET(request: NextRequest) {
  ...
  const prompt = `사용자가 자영업 창업 업종을 검색 중입니다. 입력과 가장 정확하게 매칭되는 NTS 100대 생활업종 **1개만** 골라주세요. 어설프게 비슷한 거 여러 개 X — 확신 있는 정답 1개 또는 빈 배열.
...
규칙:
- 위 리스트에 있는 정확한 이름만 사용
- **딱 1개의 가장 정확한 매칭만 반환** (애매하면 빈 배열)
- "필라테스" → "스포츠교육기관" (다른 후보 X)
- "요가원" → "스포츠교육기관"
- "베이커리" → "제과점"
- "막걸리집" → "간이주점"
- "키즈카페" → "커피음료점"
- 매칭 확신 없으면 matches: []
- reason: 한 줄 ("필라테스는 운동을 가르치는 시설이라 스포츠교육기관에 해당해요")
...
{"matches": [{"name": "...", "reason": "..."}]}`;
  ...
  try {
    ...
    // 검증: 리스트에 있는 이름만 + 최대 1개
    const validMatches = (parsed.matches || [])
      .filter((m) => upjongList.includes(m.name))
      .slice(0, 1);
